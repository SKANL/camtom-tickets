...
| F4: App owns FilterState with excludeStates | Any filter recomputes; defaults from metadata | Metadata fail → empty defaults |
...
| F5: AND logic with exclude after include | P1+Started=intersection; excludeStates removes subset | ExcludeStates empty → no-op |
| F6: text search (ci substring) | "auth" matches | Empty→all |
| F7: useMetadata fetch + default ID resolution | Cache + ID resolution | Fail→stale defaults; both fail→error |
| F8: Default Filter Initialization | excludeStates=[doneId,prSentId], labels=[ticketId] | Metadata not loaded → empty arrays |
| F8a: defaults resolved on metadata ready | Applied on stale-while-revalidate | — |
| F8b: case-insensitive substring name matching | First match resolved | No match → empty (graceful) |
| F9: Filter Chain Exclusion Step | State in exclude → excluded | ExcludeStates empty → pass through |
| F9a: state in both include + exclude | Excluded (exclusion wins) | — |
